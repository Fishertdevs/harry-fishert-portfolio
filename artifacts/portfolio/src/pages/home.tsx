  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="transform -rotate-90">
        ...
        <motion.circle
          cx={size / 2} cy={size / 2} r={radius} stroke={color} strokeWidth={strokeWidth} fill="transparent" strokeLinecap="round"
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <motion.span className="font-bold" style={{ color, fontSize: size < 80 ? '0.875rem' : size < 120 ? '1.125rem' : '1.5rem', textShadow: color === "#F7DF1E" ? "0 0 2px rgba(0,0,0,0.2)" : "none" }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
          {percentage}%
        </motion.span>
      </div>
    </div>
...
function StackCarousel({ language }: { language: string }) {
  ...
  const stackData = [
    { name: "Python", percentage: 85, color: "#F97316", tags: language === "es" ? ["IA", "Automatización", "Testing", "Backend"] : ["AI", "Automation", "Testing", "Backend"] },
    { name: "TypeScript", percentage: 90, color: "#3178C6", tags: language === "es" ? ["Tipado", "Escalable", "Seguro", "Moderno"] : ["Typed", "Scalable", "Safe", "Modern"] },
    { name: "JavaScript", percentage: 90, color: "#F7DF1E", tags: language === "es" ? ["Frontend", "APIs", "Interactivo", "Dinámico"] : ["Frontend", "APIs", "Interactive", "Dynamic"] }
  ]

  useEffect(() => {
    const interval = setInterval(() => { setMobileSlide((prev) => (prev + 1) % stackData.length) }, 4000)
    return () => clearInterval(interval)
  }, [stackData.length])

  return (
    <motion.div key="slide2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.5 }} className="flex flex-col items-center text-center">
      <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-2 md:mb-4 text-gray-900 dark:text-white">{language === "es" ? "Mi Stack de Trabajo" : "My Work Stack"}</h2>
      <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto text-xs sm:text-sm md:text-base mb-4 md:mb-6">{language === "es" ? "Implementación de arquitecturas modernas para el desarrollo de soluciones integrales, escalables y orientadas a resultados." : "Implementation of modern architectures for the development of comprehensive, scalable and results-oriented solutions."}</p>
      ...
      <div className="hidden md:flex justify-center items-start gap-8 lg:gap-12 w-full max-w-4xl">
        {stackData.map((stack, index) => (
          <motion.div key={stack.name} className="flex flex-col items-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.2 }}>
            <h3 className="text-lg lg:text-xl font-bold mb-3" style={{ color: stack.color, textShadow: stack.name === "JavaScript" ? "0 0 1px rgba(0,0,0,0.15)" : "none" }}>{stack.name}</h3>
            <CircularProgress percentage={stack.percentage} color={stack.color} size={120} />
            <p className="text-xs text-gray-700 dark:text-gray-300 text-center mt-3 max-w-[180px]">{stack.tags.join(" | ")}</p>
          </motion.div>
        ))}
      </div>
      <div className="md:hidden w-full max-w-xs">
        <AnimatePresence mode="wait">
          <motion.div key={mobileSlide} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3 }} className="flex flex-col items-center">
            <h3 className="text-lg font-bold mb-3" style={{ color: stackData[mobileSlide].color, textShadow: stackData[mobileSlide].name === "JavaScript" ? "0 0 1px rgba(0,0,0,0.15)" : "none" }}>{stackData[mobileSlide].name}</h3>
            <CircularProgress percentage={stackData[mobileSlide].percentage} color={stackData[mobileSlide].color} size={100} />
            <p className="text-xs text-gray-700 dark:text-gray-300 text-center mt-3 max-w-[200px]">{stackData[mobileSlide].tags.join(" | ")}</p>
          </motion.div>
        </AnimatePresence>
        <div className="flex justify-center gap-2 mt-4">
          {stackData.map((_, index) => (
            <button key={index} onClick={() => setMobileSlide(index)} className={`h-1.5 rounded-full transition-all duration-300 ${index === mobileSlide ? "w-6 bg-primary" : "w-1.5 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400"}`} />
          ))}
        </div>
      </div>
    </motion.div>
...
export default function Home() {
  ...
  useEffect(() => {
    const interval = setInterval(() => { setHeroSlide((prev) => (prev + 1) % totalHeroSlides) }, 15000)
    return () => clearInterval(interval)
  }, [])
      ...
      <main className="flex-1">
        <section id="home" ref={heroRef} className="relative flex items-center justify-center py-16 md:py-20 overflow-hidden">
          ...
          <motion.div className="relative z-10 px-4 max-w-6xl mx-auto w-full" variants={containerVariants} initial="hidden" animate={isVisible ? "visible" : "hidden"}>
            <AnimatePresence mode="wait">
              {heroSlide === 0 && (
                <motion.div key="slide1" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.5 }} className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
              ...
              {heroSlide === 1 && <StackCarousel language={language} />}
            </AnimatePresence>
            <div className="flex justify-center gap-2 mt-8">
              {[0, 1].map((index) => (
                <button key={index} onClick={() => setHeroSlide(index)} className={`h-2 rounded-full transition-all duration-300 ${index === heroSlide ? "w-8 bg-primary" : "w-2 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400"}`} />
              ))}
            </div>
            {showCVPreview && (
